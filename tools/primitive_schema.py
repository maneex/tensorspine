"""`--document primitive-schema`: one JSON Schema per primitive version, generated from its argument
declarations (§4.1, §4.6). Non-normative, and not a derived product (§7): the validator stays the
authority for whether an instance's arguments satisfy a primitive. This is a rendering, like
`--document primitive-library` — a convenience for a tool that wants to check a bag of arguments with a plain
...
def schema_for(name, version, definition):
    """The JSON Schema of one primitive version's arguments."""
    schema = {
        '$schema': 'https://json-schema.org/draft/2020-12/schema',
        '$id': f'https://tensorspine.dev/primitive/{name}/{version}.json',
        'title': f'{name}@{version} arguments',
        'description': f'Generated from the {name} {version} primitive (non-normative; the validator '
...
def render(cat):
    """{`name@version`: schema} for every non-template primitive version of the primitive_library."""
    out = {}
...
def run(primitive_library_bases, schema_dir, output=None):
    """Write one schema per primitive to a directory (`-o DIR`), or the whole map as one JSON object
    to `-o FILE` or stdout. Returns a CLI exit status."""
    cat = primitive_library_mod.load(*primitive_library_bases, schema_dir=schema_dir)
    schemas = render(cat)
    if output and os.path.isdir(output):
        for cid, schema in schemas.items():
            with open(os.path.join(output, cid.replace('@', '_') + '.json'), 'w', encoding='utf-8') as f:
                ...
                f.write('\n')
        print(f"  {len(schemas)} primitive schema(s) -> {output}/", file=os.sys.stderr)
    else:
