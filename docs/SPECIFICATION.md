...
| **Invariants** | The relations the resolved arguments must satisfy for the occurrence to be admissible — `heads` a multiple of `kv_heads`, `top_k` no greater than `experts` (V8, §6). A domain constrains one argument; an invariant relates several. |
...
| **V8** | The contract's declared invariants hold on the resolved arguments, defaults applied — `heads mod kv_heads = 0`, `top_k ≤ experts`. Meaningless combinations are excluded by an invariant, never merely by a missing guard (I11); an invariant reading an argument refused by V3 is skipped, one reading an argument that may be absent is guarded by a `present` test (§4.3). |
...
| **major** | An existing valid occurrence would denote, derive or compute something different, an implementation of the previous version breaks, or a previously valid occurrence is now refused — a new domain or invariant, a narrowed enum, a field made required: a **restriction** is major, because a document that resolved no longer does. |
