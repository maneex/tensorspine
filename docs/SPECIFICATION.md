...
> Represent every in-scope model as a **finite graph of parameterized primitive instances**, then
> derive all other information from the primitives' primitives.

*TensorSpine 2.0 — language specification, revision of 29 August 2026. The language version is the
`tensorspine/2.0` tag every document and schema carries; primitive library units carry their own versions
...
This document specifies what a model definition can express and what a primitive definition must contain
...
- **Visits are data-independent.** Which instances an element visits, and how often a state is
...
- **The state evolution rules are three.** A state evolves by `append`, `window` or `fixed` (O5.2); a state
  that evolves otherwise — a retained prefix kept beside a sliding window — has no representation
  and is a new evolution: a capability every consumer must implement (§8.1), not a rearrangement.

**Coverage** is closure of that class over the available primitives:

> Every graph of the class above whose instances use available primitives — any wiring, any sharing
...
about the primitives. A node with no primitive is not a counter-example but a new primitive (§8.1),
and a document is only as expressive as the primitive library it pins. The reference corpus
...
primitive's port set is static, so a variable number of operands is expressed by chaining or by
...
<a id="12--governing-principle"></a>

### 1.2 — Governing principle

> **The model declares causes. Primitive Definitions derive consequences.**
...
<a id="model-document"></a>

<a id="2--model-document"></a><a id="2--model-definition"></a>

## §2 — Model Definition

A model definition contains exactly:

| Element | Content |
|---|---|
| **Identity** | A stable, authoritative model identifier (O1.2), and, for a template, the version of this representation (§4.6). |
| **Primitive Library bases** | The primitive library bases whose units the instances pin, as locations relative to the document; a base that does not exist is a rejection (V1), and two bases carrying one identity with different contents are a conflict (V1). Primitive Definition identity is per instance, `{name, version}`, immutable (O1.1, §8.2); there is no primitive library-wide version. |
...
| **Instances** | Graph nodes (§3). |
| **Compositions** | Finite indexed families of instance sites whose expansion is normative (§5). |
...
<a id="21--quantities-o21-o22-o23-o04"></a>
...
- **Dimensional type:** a cardinality (non-negative integer); a ratio or real hyperparameter bounded
  by primitive; a physical quantity with a **unit** (bytes, elements, tokens, seconds, operations);
...
Load variables such as batch size and sequence count do **not** belong in the model definition.

<a id="22--derivation-algebra-and-qualified-values-o01-o02-o03-o05-o06"></a>
...
**O0.3 —** A derived quantity of a model definition is exact and its provenance is the expression
...
primitive states (§4.1) and the derived products (§7).

**O0.5 — Qualified values.** A value stated by a primitive correction (§4.1), by a sparsity bound
...
normative interface. This applies to the size algebra, the primitive-condition language in §4.3, and
the expansion rules in §5. Calling a representation “symbolic” does not make it decidable.

<a id="23--public-inputs-and-outputs-o81-o82-o83-o42"></a>

### 2.3 — Public inputs and outputs *(O8.1, O8.2, O8.3, O4.2)*

Each public **input** declares the instance ports it feeds — one or more — and its indexing
...
public **output** names one instance port and states whether it is generative; its domain is
...
<a id="3--occurrences"></a><a id="3--instances"></a>

## §3 — Instances

An instance is a graph node. The language keeps these five categories distinct:
...
<a id="31--identity-o21-o35"></a>

### 3.1 — Identity *(O2.1, O3.5)*

Every instance has a stable identifier, unique after expansion (§5.2), and belongs to one or more
named, addressable families. Families allow sets of instances to be referenced without
enumeration.

<a id="contract-reference-o1.1-8.2"></a><a id="primitive-reference-o1.1-8.2"></a>

<a id="32--contract-reference-o11-82"></a><a id="32--primitive-definition-reference-o11-82"></a><a id="32--primitive-reference-o11-82"></a>

### 3.2 — Primitive Definition reference *(O1.1, §8.2)*

An instance references a primitive through an immutable identity, `{name, version}` with a semantic
version (§8.2). Capability rejection (§8.1) answers “Can this consumer execute it?”
Primitive Definition identity answers “Which exact meaning did the author reference?” Neither replaces the
other.

<a id="33--arguments-o91-o94"></a>
...
`instance.port → instance.port`. Producer-consumer flow is never inferred from implicit mutation
of a named flow. Explicit edges determine the values live at any graph split and therefore the graph split's
**logical payload** (O4.5), which is not actual traffic.

**Parameter-tensor bindings** *(O6.2)* identify the logical tensors consumed by each instance and
any sharing between instances. With tied embeddings, one physical tensor satisfies two logical
...
shape axes — local name `multiplicity`, the primitive library's `storage.multiplicity`, extent m — which every form
addresses as it addresses a shape axis: a `stack` over it takes one location per copy, a `tensor` the
copies whole as [m, …]. The storage axis belongs to the stored form alone: it is in no primitive shape, so
...
**Parameter identities** are compatible only between slots whose primitives declare them
`shareable`, whose roles are listed in each other's sharing rules, and whose shapes are equal under
V4 (V15).

**State identities** declare when instances use the **same storage**. Sharing is a topological fact
that no primitive definition can know: the primitive derives a state descriptor, while the graph
declares which instance reads another instance's state.
...
> between fragments follows from the primitive's carrying condition and the input's fragmentation
...
<a id="35--flow-multiplicity-o41-o43"></a>
...
instance, never an annotation (O4.3).

<a id="primitive-semantic-contracts"></a>

<a id="4--primitive-semantic-contracts"></a><a id="4--primitive-semantic-primitives"></a>

## §4 — Primitive semantic primitives

A primitive definition is the immutable, versioned semantic definition referenced by an instance.
It declares the primitive's arguments and derives its ports, logical tensors, state, effects, costs,
and valid partition options. Primitives provide coverage: a new primitive can be added without changing the
language.

<a id="contract-contents-o9.2-semantic-part"></a><a id="primitive-contents-o9.2-semantic-part"></a>

<a id="41--contract-contents-o92-semantic-part"></a><a id="41--primitive-definition-contents-o92-semantic-part"></a><a id="41--primitive-contents-o92-semantic-part"></a>

### 4.1 — Primitive Definition contents *(O9.2, semantic part)*
...
| **Arguments** | Types, allowed values, required status, declared defaults, conditional presence (`present_when`), and numeric **domains** — an interval or set an argument's value must lie in, checked at every call site (V3). |
...
Every primitive has a primitive; only primitives with state ports have a state primitive. Token
...
A primitive never contains a generator, a backend, a kernel, a memory layout, a fusion, a workspace,
...
generator, executes the witness of every primitive version (above); being the authority for the
computation does not make its costs, layouts or fusions the primitive's.

<a id="42--computation-may-be-delegated-identity-may-not"></a>

### 4.2 — Computation may be delegated; identity may not

An implementation may provide computation, but the primitive still fixes semantic identity, ports,
...
The delegate is checked against the witness (§4.1): an implementation of a primitive version
...
primitive's meaning, not about an oracle outside the language.

The computation provider may itself be a parameterized model definition: the template primitive
defined by §4.6. Delegation relaxes none of these obligations.

<a id="43--state-derivation"></a>

### 4.3 — State derivation

For each state port, the primitive declares:
...
- **Payload.** Each storage component has a shape, type, and multiplicity. One state may contain
  ...
  evolution's and whose span is a modulator; the whole state for `fixed`. A payload therefore carries a
  `sequence.position` axis only if every rule of its port is `fixed`; a primitive that does otherwise
  is refused when the primitive library is loaded.
- **Derived properties.** These are consequences of arguments and never appear in the model:

| Property | Reference | Meaning |
|---|---|---|
| **Evolution evolution** | O5.2 | How extent evolves: append, bounded window, or fixed size. |
| **Indexing domain** | §5.3 | The stream along which the state grows: the instance's own (`self`) or that of one of its input ports (`port`); the evolution alone is ambiguous. |
...
| **Sharing** | O5.3 | The granularity at which sessions may share the state. `by_position`: entries are per position; sessions whose streams are identical up to a position may share that position's entry, so a shared prefix is shared entry by entry. `by_source`: the whole state; sessions whose complete source streams are identical share it. `within_span`: entries are per position and only the last `span` are retained; sessions whose streams are identical up to the right edge may share the retained positions, and no older position is served through the state — by another session or by another instance reading the same identity. `at_fork_point`: the whole payload, read and written as one; a session forking another copies it at the fork and shares nothing after, since every element overwrites it. Equality of content is what a sharing runtime proves; the granularity says which entries exist to share. |
...
| **Writing** | O5.4 | A condition over the arguments under which the instance writes the port (`written_when`); absent, it always does. Among the members of a state identity exactly one writes (V20); the others read the storage the writer fills — cross-layer key/value sharing is this and nothing else. |
...
primitive that does otherwise is refused when the primitive library is loaded. Rules are ordered; the first
matching rule wins; a present state port that no rule matches for an instance's arguments is a
...
<a id="44--information-supplied-by-the-graph"></a>

### 4.4 — Information supplied by the graph

> `instantiated states = primitive(primitive, arguments) × graph instances × state identities`

A primitive supplies the descriptor; the graph supplies instance count and relationships. The model
...
indices). Whether a state survives between fragments follows from the primitive's carrying condition
...
the graph values live across a graph split and determines that graph split's logical payload.

| Element | Reference | Authority |
|---|---|---|
| **Instance key** | O5.5 | **Derived:** the identity's indices × the primitive's `key_axes` (session, branch). Sharing is declared by listing several members under one identity, never by a relation over keys; exactly one member writes an identity instance — the one whose port's `written_when` holds — and the others read it (V20). |
| **Liveness evolution** | O5.5 | **Derived:** one class per distinct instance key. How many classes are active at once is deployment intent (§10.3): the model fixes the class structure, never the count, and never the raw product of dimensions. |
...
A model definition therefore never references a deployment or request fact: there is no open
...
<a id="45--structured-sparsity-o66"></a>

### 4.5 — Structured sparsity *(O6.6)*

When a primitive activates only some of its parameters per element, its primitive declares one or
...
<a id="template-contracts"></a>

<a id="46--template-contracts"></a><a id="46--template-primitives"></a>

### 4.6 — Template primitives

A primitive may be implemented by a **template**—a parameterized model definition—instead of a consumer
capability. The primitive library designates the template; it does not describe it:
...
A template primitive's **ports** are the template's public inputs and outputs, with their domains;
...
id (V1). Template instances become instances of the calling graph with identifiers prefixed by
the invoking instance (§5.2).

The template is not the primitive identity: two templates may realize one semantic identity, and a
template name identifies a realization, not a meaning. The realization is nevertheless pinned —
the primitive names the template version, and the template carries that version — and how template
and primitive versions relate is defined in §8.2.

A parameterized document denotes a family, not one graph. Its external-source quantities are
primitive arguments supplied by an **assignment**, except those with declared defaults:

> For every admissible assignment, a parameterized document denotes exactly one graph.

Admissibility must be decidable at the call site. The primitive therefore exposes the declared types
...
An external quantity may declare a default in the template, which the primitive exposes as a
...
Delegation removes the need for a new consumer capability (§8.1), but none of §4.1's primitive
contents. Logical tensors, state ports, logical costs, and semantic partition options are derived from the
...
The primitive citation graph is acyclic. A template may cite another template primitive, but a
...
<a id="51--the-expanded-graph-is-authoritative"></a>

### 5.1 — The expanded graph is authoritative

A document denotes one finite instance graph. Families, stacks, trunks, repetitions, and patterns
are syntactic sugar with the expansion defined here. All validity rules apply to the expanded graph.

A **composition** is a named, finite, indexed family of instance sites. Expansion evaluates
its index ranges and conditions, then emits ordinary instances with deterministic identifiers; a
composition is not a runtime node or an independently nested graph.

<a id="52--deterministic-expansion"></a>
...
2. **Identifiers.** A root instance is identified by its name; a generated instance by
   `<composition>/<site>[<index>=<value>,…]`, indices in name order; an instance of a template by
   ...
   one-to-one correspondence of instances preserves primitives, arguments, families, every value
   edge, the grouping of slots into parameter, constant and state identities, the interface names
   and the identity names.
3. **Resolved references.** Every quantity, instance, port, family, identity, stream, primitive library
   base and template reference resolves (V1). A binding is emitted only where every instance it
   names is emitted; an instance absent by its guard is not a reference failure, an unknown name
   or an index outside a composition's ranges is one. A rule naming several members is emitted
   where all are.
4. **Set semantics.** The expanded graph is a set of instances, edges, families and identities;
   expansion is deterministic as a set, whatever the order of members in the document. Any listing
   is conforming; the canonical listing orders instances by identifier, edges by (source,
...
6. **Guards:** a site or a binding may carry a `when` condition over the document's quantities and
   ...
   guard would otherwise drop instances silently (I7). Guards are how a periodic layer pattern is
...
7. **Scoped bindings:** a composition may carry bindings written against its own sites. Each such
   ...
   index ranges and whose site endpoints select the generated instance at the current indices,
...
- primitive-level `present_when` is evaluated from resolved primitive arguments and controls whether
  a port, parameter, constant, or state slot exists;
- `when` on an ordered primitive rule is also evaluated from resolved arguments and controls whether
...
<a id="53--indexing-domains-streams-and-fragmentation"></a>
...
A primitive port declares a kind, or `inherit`. The **instance's own domain** is the common domain
...
it and takes the edge's stream; an output port declaring a kind has that kind and the instance's
stream unless a transform says otherwise; a port declared `inherit` from an input port has that
port's domain.

An instance whose every input port is transformed has no own domain; its outputs' domains come
from its transforms.

A **transform** relates a port to the instance's domain: `merge` — the output carries the input
port's stream at the output's kind (which may be the same kind), one element per `factor` input
elements; `align` — the input port carries another domain and the output stays in the instance's
domain; `insert` — the input port's elements enter the instance's stream, which the output keeps.
...
destination is the `from_port` of a transform of its primitive. A public input is an edge like any
other.
...
instance whose state is carried must sit on a fragmented stream (V16); nothing is declared twice.

An instance that reads across positions of a fragmented stream (its primitive's `across_positions`
...
condition** is the primitive's, and the language states it by invariant (V8): on a fragmented
stream, a primitive's reading depends on no element later than the one it produces — it looks back,
...
| Rule | Requirement |
|---|---|
| **V1** | Every quantity, instance, port, family, identity, stream, primitive library base and template reference resolves (I1). Two bases carrying one `{name, version}` with different contents are a conflict, not a choice. |
| **V2** | Every required primitive argument is present; every undeclared argument is rejected; declared defaults are applied before checking. |
| **V3** | Types, enums, record fields, units and domains conform, for arguments and for quantities, derived ones included; an enum value outside its declared set is rejected; a number literal is an instance of its declared type (`32.0` is not a cardinality); a physical value in `tokens`, `elements`, `bytes` or `operations` is a whole number, only `seconds` being real; a quantity selecting a dtype is an enum over dtypes. A primitive argument's domain is a scalar literal or a reference to a required-or-defaulted argument, checked here after the type. |
...
| **V7** | Bindings are total and unique: every input port of an emitted instance is fed exactly once; every parameter, constant and state slot present under its `present_when` is bound exactly once; a slot absent by its condition is bound by nothing. A present slot's declared multiplicity resolves to a positive integer: a slot with no copies is absent by its condition, never by arithmetic (§3.4). |
| **V8** | The primitive's declared invariants hold on the resolved arguments, defaults applied — `heads mod kv_heads = 0`, `top_k ≤ experts`. Meaningless combinations are excluded by an invariant, never merely by a missing guard (I11); an invariant reading an argument refused by V3 is skipped, one reading an argument that may be absent is guarded by a `present` test (§4.3). |
...
| **V13** | Every output port of an emitted instance is consumed by an edge or exposed by a public output. |
...
<a id="7--required-derived-products"></a>
...
instance is evaluated in an invocation when every input port receives elements, excepting a
port that is the source of an `insert` transform of its primitive, and a port whose elements a
state of the instance indexed by that port already holds in full (an `append` state; a `window`
holds a suffix and exempts nothing); an instance not evaluated delivers
...
consuming one generated element (decode). A **graph split** is a partition of the emitted instances into
two blocks; it is **valid** when every crossing edge is directed from the first block to the second.
The **payload** of a graph split is the set of values on its crossing edges — the values live at the graph split —
...
evaluates a member instance: an `append` or `window` state indexed by its own stream is written
...
A valid document and its referenced primitives make all products below computable without inference
code or human knowledge of a named mechanism:

| Product | Content |
|---|---|
| **D1** | **Derived Computation Graph:** instances, edges, and families; per instance, whether its primitive reads across positions (§4.1). |
| **D2** | **Derived Value Shapes and Lifetimes:** the value and shape inventory; the payload of every valid graph split — the values live at it, sized per invocation; the peak of live values along one order of the graph, the activation peak of an invocation; and the fragment alignment of every fragmented stream (§5.3). |
| **D3** | **Derived Parameter Tensor Inventory:** shapes as stored (a declared multiplicity leading, §3.4), sharing, and total count; the role, selected dtype and sensitivity of every tensor; when the document locates its weights, the evaluated location of every tensor. |
| **D4** | **Derived State Inventory and Behavior:** descriptors, instances, keys, the writer of each identity instance, state liveness, visits per phase, and permitted operations. |
| **D5** | **Derived Logical Resource Requirements and Costs:** parameters, activations, state per element, computation — derived from the inventory and the declared corrections (§4.1) — and the payload crossing each valid graph split per invocation. |
| **D6** | **Derived Decomposition Options:** how the expanded graph may be taken apart, without taking it apart. The valid graph splits it names — every layer prefix of a composition and every family, closed under ancestors — each with its first block, the values crossing it (D2, D5), and the state identities it separates: those with members on both sides, their writer's side, and which readers on the side away from the writer may need positions the state no longer holds — every far-side reader of a `window` identity, which no state serves past its span (§4.4). A split is a line, not a decision: any prefix of D1's order is valid too, and which line is useful is deployment's (§10.3). For every instance, the partition options its primitive declares with their communications and granularity; a flattened axis without factors is reported as information loss (O5.10). Partition options are declared per instance; their consistency across instances — the residual width through norm, add and feed-forward, a head partition aligned to the KV groups of its layer — is compilation's (§10.3), the axis identities on D1's edges being what a compiler aligns. |

These are compilation outputs. Their encoding and the decisions that consume them are outside this
specification.

<a id="8--extension-and-rejection"></a>

## §8 — Extension and rejection

<a id="81--extension-kinds"></a>
...
| **Template primitive** (§4.6) | No breakage. | No new capability if the transitive closure of its template is supported; otherwise the cost is exactly the missing primitives, never the composite. |
...
Only templates can have zero consumption cost, which is how the primitive library remains open. The
first three extensions add vocabulary that must be implemented. A template composes existing
vocabulary. Its cost is computed from the transitive closure of cited primitives; a template that
cites an unsupported primitive merely hides that cost.

Every primitive, argument, field, reference, or combination that a primitive cannot interpret is
...
the primitive are both errors. Otherwise a consumer could silently accept information it does not
understand.

<a id="82--identity-and-versioning"></a>

### 8.2 — Identity and versioning

Capability rejection and primitive identity are orthogonal. A published `{name, version}` is one
...
|---|---|
| **patch** | The declared meaning is unchanged — the unit's prose and cited sources — and no product (D1–D6) changes for any existing instance: documentation, notes, a template rewritten to the same family. The witness may change only where it was wrong against the declared meaning or against the integration fixtures of a model that uses the primitive (§10.2); its unit fixtures are then re-recorded, and a conformer that matched the old witness may now disagree — the accepted cost of a patch, stated in the version note. |
| **minor** | Additive only: an argument with a declared default, an optional port, a new enum value, a new partition entry, a correction that did not apply before. Every existing instance denotes, derives and computes the same, and no implementation of the previous version breaks. |
| **major** | An existing valid instance would denote, derive or compute something different, an implementation of the previous version breaks, or a previously valid instance is now refused — a new domain or invariant, a narrowed enum, a field made required: a **restriction** is major, because a document that resolved no longer does. |
...
file; a primitive pins one template version; a template rewrite is a patch of the primitive pinning
the new version, and earlier primitive versions keep resolving. "Same family" is asserted by the
digit and may be checked on any assignment by expansion (§5.2 rule 2). **Specification stage:**
until the first release of a primitive library, its units are edited in place at `1.0.0`.

A compatible addition need not change the model-language version, but every prior primitive identity
must remain immutable. Otherwise instances cannot reference reproducible meanings.

<a id="83--benefit-of-closure"></a>
...
<a id="9--invariants-and-non-requirements"></a>
...
<a id="10--coverage-and-rejection-cases"></a>
...
<a id="102--required-rejection-cases"></a>
...
identity (V15); a carried state on a stream that is not fragmented (V16); an instance
...
A primitive library is refused when loaded if a primitive compares or computes with an optional argument that
has no default outside a `present` test of it; if a precision role's default is not admissible; if
a sparsity policy names an argument or input port the primitive does not declare; if a state payload
carries a `sequence.position` axis under a rule that is not `fixed` (§4.3); if a template
primitive's template is absent from the declared location or carries another name, version or id.

**Release rule.** A primitive library is released — its base tagged — only when every primitive version it
...
the primitive's declared meaning — its description and the sources it cites — and the integration
fixtures recorded from the delivery implementation of a model that uses the primitive. A witness
corrected toward that authority is a patch of the primitive (§8.2); a witness that changes what a
primitive means is a new identity.
...
<a id="103--explicitly-separate-concerns"></a>
...
<a id="appendix-a--requirements"></a>
...
| **O0.3** | A derived quantity of a model is exact, reads only declared quantities, resolves acyclically and conforms to its declared type and domain; epistemic status and provenance are declared on primitive costs and derived products. |
...
| **O1.1** | A primitive is referenced through an immutable identity, `{name, version}`, whose meaning never changes (§8.2). |
| **O1.2** | A model definition carries a stable, authoritative identifier. |
| **O1.3** | Every published primitive version has one witness — a reference implementation supplied with the primitive, executed by the reference generator and bound to the identity by its manifest with a tolerance per compute dtype — that is the authority for what the primitive computes; every other implementation is a conformer, checked against it (§4.1, §4.2). |
| **O2.1** | Every quantity and every instance has one stable identifier, unique after expansion. |
...
| **O3.5** | Every instance belongs to one or more named families, addressable without enumeration. |
...
| **O4.3** | Residual multiplicity other than one is explicit, and the combining operation is a defined primitive. |
| **O4.4** | Value flow is declared by explicit port-to-port edges, never inferred from ordering or from mutation of a named flow. |
| **O4.5** | The logical payload of a graph split is the set of values live across it, determined by the explicit edges; it is not measured traffic. |
| **O5.1** | A state payload may hold several components of different shapes and types. |
| **O5.2** | Each state port derives an evolution rule: append, bounded window, or fixed size. |
...
| **O5.5** | The instance key of an allocation is derived from the identity's indices and the primitive's key axes; liveness is one class per distinct key, never the raw product of dimensions. |
...
| **O6.1** | Every primitive derives its logical tensor inventory from its arguments: shapes, roles and sharing rules. |
| **O6.2** | Parameter bindings identify the logical tensors each instance consumes and every sharing between instances; a tied tensor is counted once. |
...
| **O7.1** | A primitive declares the axes along which partition preserves meaning, the logical communication each partition implies — several when the partition admits several patterns — and the granularity a shard keeps whole along the axis. |
| **O8.1** | Every public input names one or more instance ports and declares its kind and stream; every public output names one instance port and whether it is generative. |
...
| **O9.2** | A primitive declares the elements of §4.1: arguments, ports, logical tensors, state ports, effects, corrections, partition options and transforms. |
| **O9.4** | An argument is a scalar, an enum, or a record of such values; inline numeric tensors are forbidden. |
| **O9.5** | A primitive declares, as a condition over its arguments, whether the primitive reads positions of its stream beyond those of the element it produces; an instance that does so on a fragmented stream carries a state across the fragments (V18). |
