...
| `values` | One entry per value, keyed by its source `node.port`: `to` (every consumer), `shape` (axis identity and evaluated extent, factors when declared), `role`, `dtype` (the role's default: the language selects dtypes on identities, not on activations), `elements`, `bytes_per_element`, `domain` `{kind, stream}`, `count`. |
