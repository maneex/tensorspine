...
/**
 * `record[name] = value` on a plain object built from a document.
 *
 ...
 * disappear from the reading. It is defined as a data property instead. The object keeps
 * `Object.prototype` all the same, because a validator that reads `a.valueOf` — Ajv's deep
 * equality does — crashes on an object without one.
 *
 * The rule is stated here, where the reading is made, and every record the core builds from a
 * document goes through it: `toPlain` and `toPython`, the argument maps of an instance, the
 * hoisted bindings of a normalised document.
 */
export function put<T>(record: Record<string, T>, name: string, value: T): void {
  if (name === '__proto__') {
...
export function toPlain(value: JsonValue): unknown {
  ...
  if (isJsonObject(value)) {
    ...
    for (const member of value.members) put(plain, member.name, toPlain(member.value));
    return plain;
  }
  return value;
}
