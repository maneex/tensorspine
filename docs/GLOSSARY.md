...
The untagged model format retains `tensorspine/2.0`. See the
[conversion instructions](../README.md#converting-earlier-model-definitions)
for documents that use the earlier field layout.

### Computation Graph

The expanded computation, represented by D1. Its Nodes represent Primitive
Instances and its Dataflow Edges carry computational values between ports.

### Primitive Reference

The `primitive` citation containing the name and version of a Primitive Definition.

### Primitive Instance Site

A declaration in `instances`, at the root or within a composition. A generated
or guarded site emits zero or more concrete Primitive Instances during expansion.

### Node

A Primitive Instance as represented in the Computation Graph, with its graph identifier.

### Dataflow Edge

A connection carrying a computational value between an Output Port and an Input
Port. Bindings also associate parameters, constants and state identities with
their category-qualified Parameter Slots, Constant Slots and State Slots.

### State Evolution Rule

A rule selecting state behavior through `evolution`: `append`, `window` or
`fixed`. The rule also supplies its access and indexing geometry; renaming the
selector does not change its state transitions.

### Model Segment

An actual chosen fragment of a model. A Semantic Partition Option describes an
available decomposition; it becomes a segment only when a decomposition is chosen.

## A

### Across positions

A primitive's declaration, as a condition over its arguments, that the primitive reads positions of
...
alone. Evaluated per instance, it is D1's `across_positions`; on a fragmented stream it demands
a state carried across the fragments (V18); with the states and the streams it tells a batching
runtime which instances it may evaluate on several sessions' elements at once (harness guide
§8). See [Specification §4.1](SPECIFICATION.md#41--primitive-contents-o92-semantic-part) and
[§5.3](SPECIFICATION.md#53--indexing-domains-streams-and-fragmentation).

### Argument

A scalar, enum, boolean, or record supplied by an instance to its primitive definition. Arguments
...
[Model guide §2.3](TENSORSPINE-MODEL_JSON.md#23--instances).

### Assignment

A set of values supplied for a parameterized model definition's external quantities. An admissible
assignment turns the document's graph family into one concrete graph. See
[Specification §4.6](SPECIFICATION.md#46--template-primitives).

### Axis

A primitive library identity for a semantic dimension, such as `model.width` or `attention.heads`. A shape
...
[Specification §4.1](SPECIFICATION.md#41--primitive-contents-o92-semantic-part).
...
form, in no primitive shape. See [Specification §4.3](SPECIFICATION.md#43--state-derivation) and
...
model definition never names one, and a serving application may target several. When it must select
one, it does so through opaque physical parameters passed beside the primitive arguments; `backend`
is one optional key ([Capabilities](../generators/CAPABILITIES.md)).

### Branch

A separately admitted choice within one primitive version: an enum value, a record field, or an
...
with the primitive slots that use it. See [Specification §3.4](SPECIFICATION.md#34--bindings) and
...
A primitive's condition, declared once on a state port, under which the state survives between the
...
precision roles, and versioned primitives; it has no single global version, and one identity carried
by two bases with different contents is a conflict (V1). See
[Model guide §3](TENSORSPINE-MODEL_JSON.md#3--primitive-library-primitives) and
[Specification §8.2](SPECIFICATION.md#82--identity-and-versioning).

### Composition

A named, finite, indexed family of instance sites, with the bindings among those sites. A site
...
bindings. Expansion turns sites into ordinary instances with deterministic identifiers and scoped
...
The logical data movement implied when a semantic partition graph splits model values. Primitives describe
...
[Specification §3](SPECIFICATION.md#3--instances).

### Conformer

An implementation of a primitive version whose outputs and states agree with that version's witness
...
The immutable, versioned semantic definition of a primitive. An instance pins a primitive by name
and version; the primitive declares arguments and derives ports, logical tensors, state, effects,
costs, and valid partition options. Its reference implementation is the witness that fixes what the
primitive computes; all optimized implementations are conformers. See
[Specification §4](SPECIFICATION.md#4--primitive-semantic-primitives) and
[Model guide §3](TENSORSPINE-MODEL_JSON.md#3--primitive-library-primitives).

### Correction

A cost entry a primitive declares because the inventory rule cannot see it: a condition over the
...
contributes. See [Specification §4.1](SPECIFICATION.md#41--primitive-contents-o92-semantic-part).

### Coverage

Closure of the in-scope graph class over the primitives available in the pinned primitive library: every such
graph has at least one representation. It is not a claim that every architecture has a primitive or
that every implementation supports every primitive. See
[Specification §1](SPECIFICATION.md#1--scope-and-authority).

<a id="cut"></a><a id="graph-split"></a>
...
[§4.6](SPECIFICATION.md#46--template-primitives) and the
[primitive-library-unit guide](TENSORSPINE-PRIMITIVE-LIBRARY-UNIT.md).

### Invariant

A relation a primitive definition declares over its arguments that must hold for an instance to be
...
[Specification §6](SPECIFICATION.md#6--static-semantics) and the
[primitive-library-unit guide](TENSORSPINE-PRIMITIVE-LIBRARY-UNIT.md#5--invariants).

### Derived Products (D1–D6)

The products that can be computed from a valid model and its referenced primitives:
...
quantities and composition indices; primitive expressions read resolved primitive arguments. The two
...
§4.1](SPECIFICATION.md#41--primitive-contents-o92-semantic-part).

## F

### Family

A named, addressable grouping of instances. Families allow later derivations to refer to a set of
nodes without enumerating every expanded instance. See
...
(`fragmented`). Together with a primitive's carrying condition it decides which states survive
between fragments. Carrying a state (V18) is necessary but not sufficient for a fragmented reading:
the primitive must also look back only, never forward — the sufficient condition, stated by
...
**unit fixture** records one primitive witness on generated inputs, parameters and states; every
conformer runs it. An **integration fixture** records a delivery implementation at a model's valid
graph splits and states; every implementation running that model compares against it. See the
...
A repository executable that builds and runs a model graph—or an extract between valid graph splits—over
primitive implementations. The reference generator is the repository's target generator: it runs
the primitive reference implementations and carries their witnesses. The ZML generator is an
example conformer. A serving application does not embed a TensorSpine generator; it implements the
subset of TensorSpine primitives it supports and may consume TensorSpine's derived facts and tooling
...
An explicit declaration that answers “which logical thing is this?” Primitive Definition identity pins meaning;
parameter, constant, and state identities group the slots that refer to the same logical resource;
instance identity names a graph node. Matching names or shapes do not implicitly create identity.
See [Specification §§3.1–3.4](SPECIFICATION.md#3--instances).
...
frames an encoder produced. Domains must agree on every edge (V5) unless a primitive transform says
how they relate. See
[Specification §5.3](SPECIFICATION.md#53--indexing-domains-streams-and-fragmentation).

### Interface

A public model input or output attached to existing instance ports. An input names the ports it
...
- **Value liveness** identifies graph values live across a graph split and therefore the graph split's logical
  payload (D2).
- **State liveness** is the number of distinct state-allocation equivalence classes active at once.
  ...
  primitive's `key_axes`. The count of active classes is deployment intent, not a model fact.
...
A `tensorspine/2.0` declaration of model identity (and, for a template, its version), primitive library
bases, quantities — literal, external with domain and optional default, or derived — constants,
instances, compositions with their scoped bindings, bindings with their identities and dtypes, and
public interfaces. It declares
graph-specific causes and relationships; primitives derive reusable consequences. See [Specification
§2](SPECIFICATION.md#2--model-document) and [Model guide
§2](TENSORSPINE-MODEL_JSON.md#2--the-tensorspine-20-model-definition).
...
<a id="occurrence"></a><a id="instance"></a>

### Primitive Instance

A node in the model graph. It has a stable identity, references one versioned primitive, supplies
that primitive's arguments, and belongs to one or more families. It is an invocation of a primitive,
not the primitive definition itself. See [Specification §3](SPECIFICATION.md#3--instances) and
[Model guide §2.3](TENSORSPINE-MODEL_JSON.md#23--instances).
...
A learned logical tensor whose slot, shape, and precision role are derived by a primitive. A
...
### Semantic Partition Option

A division along a primitive-declared semantic axis that preserves the primitive's meaning, with
the logical communication it implies. Every primitive states its partition options: one or more axes,
...
Of a graph split: see [Valid Graph Split](#valid-graph-split). Of a state: what one position holds, for `append` and `window`, or the
...
A named interface on a primitive definition. A value port carries a temporary graph value; a state
port refers to logical storage that persists across token invocations. Instance bindings connect
ports but do not redefine them. See [Specification §3](SPECIFICATION.md#3--instances).

### Primitive

A reusable semantic operation kind, such as an embedding, attention operation, feed-forward
operation, or residual addition. A primitive defines the primitive; an instance places one use of
it in a model graph. See [Specification §4](SPECIFICATION.md#4--primitive-semantic-primitives).

### Primitive implementation

Application-owned code that executes a primitive version or one of its branches: kernels, fusions,
layouts, collectives and data paths optimized for target hardware. A capabilities manifest states
the implemented subset; conformance means its outputs and states match the primitive's witness. It
...
A named scalar in a model definition, with a type and a literal, external or derived source. It is
...
A primitive library identity describing the semantic use of a value or logical tensor for precision policy,
such as `activation.hidden` or `attention.qkv_projection`. A role is not a dtype; its primitive library entry
...
validity and graph denotation; each primitive version's reference implementation—the witness—governs
...
primitive implementations execute the supported TensorSpine primitives with application-specific
optimizations. See [README §1](../README.md#1-why-tensorspine) and the
[harness guide](HARNESS.md).

### Sharing

The use of one logical resource by multiple sites. A primitive declares whether and at what
granularity sharing is semantically permitted; model bindings declare the actual parameter or state
identity shared by instances. Tied parameter members must be compatible: `shareable` on both
...
[Model guide §3.1](TENSORSPINE-MODEL_JSON.md#31--state-is-split-between-primitive-and-graph).
...
A primitive's declaration that only some of its parameters are activated per element: the slots and
...
Logical storage exposed through a state port and preserved across invocations. The primitive derives
...
closed evolution vocabulary is `append`, `window` and `fixed`; another evolution is a new state evolution rule.
See
...
fact, separate from the state descriptor derived by a primitive. Exactly one member writes the
storage — the one whose port's `written_when` holds (V20) — and the others read it. See
[Specification §3.4](SPECIFICATION.md#34--bindings).

### Status page

The generated, untracked page reporting primitive library and corpus state, implementation coverage and recorded
...
local name `multiplicity`, the primitive library's `storage.multiplicity`, extent the count. A location
...
domain. An input introduces the stream named after it or joins an existing one; a primitive
transform can `merge` a stream into fewer elements of another kind, `align` a port carrying another
stream onto the instance's own, or `insert` one stream's elements into another. See
[Specification §5.3](SPECIFICATION.md#53--indexing-domains-streams-and-fragmentation).

### Structural argument

A primitive argument flagged `structural: true`: it decides which parameter slots, ports or state
...
A model definition with external quantities: it denotes a family of graphs, one per admissible
assignment, and is instantiated by a template primitive. Templates live where the primitive library manifest
...
<a id="template-contract"></a>

### Template primitive

A primitive whose computation is provided by a template rather than by a consumer capability. The
primitive keeps its own semantic identity and pins the template's version; its parameters, states,
costs and partition options are derived from the expanded template. An instance of a template primitive
is an *instance*. See [Specification §4.6](SPECIFICATION.md#46--template-primitives).

### Transform

A primitive's declaration of how a port's domain relates to the instance's own: `merge`, `align`
or `insert`. Transforms also carry element counts, so a graph split's payload is sized correctly after a
...
The reference implementation supplied with one immutable primitive version and run by the reference
...
primitive's description, cited sources and integration fixtures remain the authority for whether a
...
|---|---|---|
| Model-level `when` | Model quantities and in-scope composition indices | Controls whether an instance site or binding is emitted during expansion; a binding is also absent wherever an instance it names is absent, so a site's guard is not repeated on its bindings |
| Primitive Definition element `present_when` | Resolved primitive arguments | Controls whether a port, parameter, constant, or state slot exists |
| Primitive Definition-rule `when` | Resolved primitive arguments | Selects which ordered derivation rule applies to an element that exists |

They are not aliases and cannot be moved between contexts. See
[Specification §5.2](SPECIFICATION.md#52--deterministic-expansion),
[Model guide §2.3](TENSORSPINE-MODEL_JSON.md#23--instances), and
[Model guide §3](TENSORSPINE-MODEL_JSON.md#3--primitive-library-primitives).
