...
├── backends/
│   ├── reference/                a loader and reference kernels in PyTorch: run, compare, chat
...
`backends/reference/` runs a model from its document and the checkpoint it locates, with no
...
```sh
python3 backends/reference/ref.py chat data/models/qwen3.5-4b-text.json \
        --checkpoint ~/work/perso/huggingface/Qwen3.5-4B --capacity 1024 --max-new-tokens 200
```

Slow by design on CPU; see [`backends/reference/README.md`](backends/reference/README.md) for the
